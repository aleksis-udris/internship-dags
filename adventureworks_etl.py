...
def adventureworks_etl():

    @task()
    def create_clickhouse_table(table_name, columns_wt, pk_columns):
        client = get_clickhouse_connection()

        ddl = f"""
        CREATE TABLE IF NOT EXISTS ADVENTUREWORKS_DL.{table_name} (
            {", ".join(columns_wt)}
        )
        ENGINE = MergeTree
    ...
    @task()
    def extract_postgres_data(schema, table, columns):
        client = get_postgres_connection()
        cursor = client.cursor()
        query_columns = ", ".join(columns)

        cursor.execute(f"SELECT {query_columns} FROM {schema}.{table}")
    ...
    @task
    def extract_clickhouse_data(target_name, columns):
        client = get_clickhouse_connection()

        res = client.query(f"SELECT {columns} FROM ADVENTUREWORKS_DL.{target_name}")

        client.close()

        return [list(r) for r in res.result_rows]

    @task
    def sync_database_to_lake(pk_columns, postgres_data, clickhouse_data, target_name, columns):
        client = get_clickhouse_connection()

        for row_index, row in enumerate(clickhouse_data):
            for pk_index in pk_columns:
                postgres_pk = postgres_data[row_index][pk_index]
                clickhouse_pk = clickhouse_data[row_index][pk_index]

                if postgres_pk != clickhouse_pk:
                    pk_col_name = columns[pk_index]

                    client.query(
                        f"ALTER TABLE ADVENTUREWORKS_DL.{target_name} DELETE "
                        f"WHERE {pk_col_name} = '{clickhouse_pk}'"
                    )
                    break

        for row_index, row in enumerate(postgres_data):
            for pk_index in pk_columns:
                postgres_pk = postgres_data[row_index][pk_index]
                clickhouse_pk = clickhouse_data[row_index][pk_index]
    ...
    for schema, tables in TABLE_CONFIG.items():
        for table, config in tables.items():
            target_name = config.get("target_name", table)
            columns = config["columns"]
            pk_columns = config["pk_columns"]
            columns_with_types = config.get("columns_with_types", table)

            task_zero = create_clickhouse_table.override(task_id = f"create_clickhouse_{target_name}")
            task_zero(target_name, columns_with_types, pk_columns)

            task_one = extract_postgres_data.override(task_id = f"extract_postgres_{schema}_{table}")
            postgres_data = task_one(schema, table, columns)

            task_two = extract_clickhouse_data.override(task_id = f"load_clickhouse_{schema}_{target_name}")
            clickhouse_data = task_two(target_name, columns)

            task_three = sync_database_to_lake.override(task_id = f"sync_{target_name}_from_{schema}")
            task_three(pk_columns, postgres_data, clickhouse_data, target_name, columns)
